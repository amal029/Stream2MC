 ...
 * Need to fix all the FIXME bugs, just don't have the time right 
 * now.
 */
...
public class biCriteriaScheduling {
    ...
    private static long TOTSTATES=0;
    ...
    private static stateGraph SG2 = null;
    
    private static long sTime = 0;
    private static String fName = "";
    
    private static float BOUND = 840;//the default bound
    ...
    */
    ...
    private static streamGraph sGraph = null;
    public biCriteriaScheduling (File f, List<state> startingStates, long sTime,
	float PI, float PHI,streamGraph sGraph) throws Exception{
	...
	SG2 = new stateGraph("__stateGraph__schedule_only"+fName,root);
	SG.setEdgeMode("directed");
	SG.add(root);
	SG2.setEdgeMode("directed");
	//The GXL document
	GXLDocument doc = new GXLDocument();
	doc.getDocumentElement().add(SG);
	gen = new GXLIDGenerator(doc);
	//for presentation
	GXLDocument doc2 = new GXLDocument();
	doc2.getDocumentElement().add(SG2);
	...
	SG2.getDocument().write(new File("./output","__state_graph_schedule_only"+fName));	
	}
	...
	if(!workListMap.containsKey(temp)){
	    ...
	    //DEBUG
	    // System.out.println("list size: "+list.size()+":"+update.getID());
	    if(list.isEmpty())
		list.offer(update);
	    else if(list.get(list.size()-1).getCost().get(0) <
    ...
    */
	...
	//DEBUG
	// System.out.println("state is: "+update.getID());
	// System.out.print("state name: "+temp);
	// for(String lgs : update.getUpdateGuards())
	//     System.out.println(" update processor: "+lgs);
	...
	if(!workListMap.containsKey(temp)){
		...
		if(list.size() == K){
			...
			/**
			   ...
			   FIXME:
			   There is the possibility that communication
			   nodes are being removed from here. If the
			   communication costs of 1 or more P---P are
			   the same then check if my partner
			   communication node is not on the same P---P
			   as me. (FIXED)
			*/
			if(((PI*list.get(list.size()-1).getCost().get(0))
			    ...
			    // DEBUG
			    // System.out.println("state: "+update.getID()
			    // 		       +" will not be added, list size is: "+list.size());
			    return true;
			    }
			else if(((PI*list.get(list.size()-1).getCost().get(0))
				+(list.get(list.size()-1).getECost()*PHI)) ==
			    ((update.getCost().get(0)*PI)+(update.getECost()*PHI))
			    ){
			    //DEBUG
			    // System.out.println("Trying to guess allocation of: "+update.getID());

			    boolean toRet = true;
			    /*
			      This is the case when we have equally
			      important communication links.
			      
			      If I am a communication link, then I will
			      have atleast one different update
			      state. If that is so, I need to check my
			      sibling states in the workListMap. If my
			      sibling has the same update processor as
			      me then I will allocate myself to a
			      different update processor.
			    */
			    
			    //Find out if I am a communication node
			    if(update.getID().startsWith("edge")){
				//first check if I am already in the commNodeMap
				if(commNodeMap.containsKey(temp)){
				    //then I can just get the siblings from here
				    if(doNotUseThisAllocation(update,commNodeMap.get(temp))){
					// DEBUG
					// System.out.println("state: "+update.getID()
					// 		   +" will not be added, list size is: "+list.size());
					toRet =  true;
					...
					// list.removeLast();
					state rem = list.removeLast();
					//We also need to remove this from the workList
					((LinkedList<state>)workList).remove(rem);
					toRet = false;
					}
				    }
				//yes I am a communication node
				//check if my parent is a split node
				else{
				    clearVisited(sGraph.getSourceNode());
				    ArrayList<String> mySiblings = new ArrayList<String>(4);
				    //DEBUG
				    // System.out.println("Trying to fill in commNodeMap for: "+update.getID());
				    // System.out.println("Searching for name: "
				    // 		       +update.getID().substring(0,update.getID().length()-1));


				    if(isMyParentSplitNode(mySiblings,sGraph.getSourceNode(),
					    update.getID().substring(0,update.getID().length()-1))){
					//Now look up WorkListMap to see if
					//my siblings have been allocated.
					//DEBUG
					// System.out.println("I am a split comm: "+update.getID());

					if(doNotUseThisAllocation(update,mySiblings)){
					    // DEBUG
					    // System.out.println("state: "+update.getID()
					    // 		       +" will not be added, list size is: "+list.size());
					    toRet =  true;
    ...
    /**
       Make the color path separately for presentation
    */
    private static void make_for_presentation(state sNode) {
	if(sNode.ifVisited()) return;
	if(sNode.getAttr("color") != null)
	    SG2.add(new state(sNode.getID()));
	sNode.setVisited(true);
	for(int e=0;e<sNode.getConnectionCount();++e){
	    if(sNode.getConnectionAt(e).getDirection().equals(GXL.IN)){
		GXLEdge le = (GXLEdge)sNode.getConnectionAt(e).getLocalConnection();
		state node = (state)le.getTarget();
		if(node.getAttr("color") != null)
		    SG2.add(new stateEdge(sNode.getID(),node.getID()));
		make_for_presentation(node);
		}
	    }
	}
	...
	/***
	    FIXME: IMP
	    TODO If the workList has partnerd notes, then shuffle
	    workList to put them at the start. Next, carryout a complete
	    order (!) operation by attaching the appropriate other nodes
	    onto the macro node of the parallel nodes.
	*/
	while(true){
		...
		System.out.println("Total number of states explored: "+TOTSTATES);
	
		//make for presentation
		clearVisited(root);
		make_for_presentation(root);
		...
		QM1: for(state update : curr.getUpdateStates()){
			...
			if(update.getIsJoinNode()){
				...
				//XXX: Only call this if the child is
				//not a join node
				boolean rr = false;
				if(!myChildIsJoinNode(update)){
				    rr = cont(update);
				    //DEBUG
	...
	if(s.getPartners() != null){
	    ...
	    //XXX: FIXME: Incorrect, we can have 2 rendezvous nodes
	    //mixed with one seq node
		...
		//XXX: Taking care of rendzevous-- MAYBE CALLING THE
		//RENDZVOUS NODES IS BETTER HERE
    ...
    /**
       ...
       We do not attach a node after the parallel computation, because
       we have already attached after sequential computation.
       When we have K=\infty or very large we do attach it.
       
       Soln: Is in taking those communication nodes first, which make
       the thing go in parallel, i.e., parallel computations should come
    ...
    */
    ...
    //This map is used to quickly lookup names after the first time of searching
    private static HashMap<String,ArrayList<String>> commNodeMap = new HashMap<String,ArrayList<String>>();

    private static boolean myParentSplitNode(cActor targetNode, ArrayList<String> list){
	boolean ret = false;
	for(int w=0;w<targetNode.getConnectionCount();++w){
		...
		if(le.getAttr("parallelEdge") == null){
		    if(((Actor)le.getSource()).getIsSplitNode()){
			//Yes, my parent is a splitNode!!
			//Now get My siblings
			eActor sNode = (eActor)le.getSource();
			//DEBUG
			// System.out.println("My parent split node is: "+sNode.getID());

			ret = true; //(1)
			for(int e=0;e<sNode.getConnectionCount();++e){
			    if(sNode.getConnectionAt(e).getDirection().equals(GXL.IN)){
				GXLEdge le2 = (GXLEdge)sNode.getConnectionAt(e).getLocalConnection();
				if(le2.getAttr("parallelEdge")==null){
				    if(!le2.getTarget().equals(targetNode)){
					//Then get the guard labels for all the siblings
					cActor node = (cActor)le2.getTarget();
					//there can be only 1 guard label
					//DEBUG
					// System.out.println("Adding siblings: "+node.getID()+" guard labels:"+
					// 		   node.getGuardLabels());

					list.add(node.getGuardLabels());
					}
				    }
				}
			    }
    ...
    private static boolean isMyParentSplitNode(ArrayList<String> list, Actor sNode, String name){
	// System.out.println("Searching for: "+name);
	boolean ret = false;
	
	if(sNode.iscActor()){
	    if(sNode.getID().equals(name)){
		//found the communication node I was looking for
		//now check if my parent is a splitnode
		//DEBUG
		// System.out.println("Found myself in the stream graph: "+name);

		ret = myParentSplitNode((cActor)sNode,list);
		if(ret){
		    //DEBUG
		    // System.out.println("My parent is a split node: "+name);
		    if(commNodeMap.containsKey(sNode.getGuardLabels()))
			throw new RuntimeException("commNodeMap already contains the label for: "+name);
		    else
			//put the guard label into the commNodeMap
			commNodeMap.put(sNode.getGuardLabels(),list);
		    }

		//set the hashmap
		//get my own guard label as the key
		return ret;
		}
	    }	
	sNode.setVisited(true);
	...
	for(int e=0;e<sNode.getConnectionCount();++e){
		...
		if(le.getAttr("parallelEdge")==null){
		    Actor node = (Actor)le.getTarget();
		    ret = isMyParentSplitNode(list,node,name);
		    if(ret) break;
		    }
		}
	    }
    ...
    private static boolean doNotUseThisAllocation(state me, ArrayList<String> list){
	boolean ret = false;
	//First get my update processor
	int counter=1;
	String g[] = me.getUpdateGuards().get(0).split("_");
	String myUpdateProcessor = "";
	for(counter=1;counter<g.length;++counter)
	    myUpdateProcessor += g[counter];
	    
	//DEBUG
	// System.out.println("My update processor is: "+myUpdateProcessor+" I am :"+me.getID());

	boolean tempR = true;
	for(String s : list){
	    if(workListMap.containsKey(s)){
		tempR = false;
		//this means that my siblings have already been
		//allocated to something
		//DEBUG
		// System.out.println("I have some of my siblings already allocated");

		LinkedList<state> list2 = (LinkedList<state>)workListMap.get(s);
	    T1: for(state var : list2) {
		    //DEBUG
		    // System.out.println("var is: "+var.getID());
		    for(String st : var.getUpdateGuards()) {
			// System.out.println("update is: "+st);
			g = var.getUpdateGuards().get(0).split("_");
			String stu = "";
			for(counter=1;counter<g.length;++counter)
			    stu += g[counter];
			if(myUpdateProcessor.equals(stu)){
			    //DEBUG
			    // System.out.print("They are same: "+stu+"=="+myUpdateProcessor);
			    // System.out.println(" States are: "+var.getID()+"!="+me.getID());

			    //this means one of my siblings is using the
			    //same processor as me that means I should
			    //not use this god-damn state at all
			    ret = true;
			    break T1;
			    }
			}
